...
| DSpark history SDPA | Complete SplitKV Map/Reduce execution variant plus fixed proposal capacity. | `dspark::sdpa::Selector` returns a component-local `Selection` with the variant and `DSparkGQACapacity`. |
