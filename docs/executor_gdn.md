...
every Task coordinate, so no Task value, TaskTemplate, or ABI buffer is materialized. Thread position `x = 0..31`
selects a Dqk lane and `y = 0..7` selects a V row inside the tile. For Qwen's `Dqk=128`, one thread owns four strided
state values:
...
The fragment is declared in MSL as `thread float state_fragments[4]`; the compiler normally keeps such thread-local
values in registers when possible, but physical register placement remains a compiler decision. Source loads and final
destination stores traverse four fragment rounds. In each round, the 32 x-lanes access one contiguous 128-byte span
(`32 * sizeof(f32)`); four rounds cover one 512-byte row. This describes the access pattern, not a guaranteed single
512-byte hardware transaction.

The threadblock loads its source tile into these distributed thread-local fragments, advances them over the request's
flat-token segment, writes `recurrent_output [T, Hv, Dv]`, materializes registered prefix candidates when requested, and
writes the final fragments to the destination recurrent-state slot.
...
256-thread threadblock. The number of V-dimension tiles is derived from `v_head_dim` and the configured tile instead of
being stored. The current state dataflow is:
...
Q and K are streamed from global memory/cache for each token and accumulated in lane-local scalar partials before
SIMD-group reductions. The recurrent kernel's threadgroup storage consists only of four scalars:
`q_inv_norm_shared`, `k_inv_norm_shared`, `decay_shared`, and `beta_shared`. It has no threadgroup state tile and no
threadgroup Q/K tile. This is register-oriented state residency, not double buffering.
...
recurrent state in token order for each request segment. Each Q/K-dimension lane loads its strided source-state fragment
once, keeps that MSL `thread` fragment local across the segment, and writes the final fragment to the destination slot
...
one row. One threadblock selects a request, V head, and V-dimension tile; its Q/K-dimension lanes load distributed
source-state fragments and then scan the request segment in order. For `num_tokens=1,num_reqs=1`, this is still a
one-step state update. For `num_tokens=spec+1,num_reqs=1`, it verifies the full target segment and materializes any
requested prefix candidate versions while scanning.
