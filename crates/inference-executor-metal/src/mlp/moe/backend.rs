...
#[derive(Clone, Copy)]
pub struct GatedMoEBucketedReplayInput<'a> {
    pub num_total_tokens: u32,
    pub num_active_tokens_key: ReplayParameterKey,
    pub hidden_state: &'a Buffer,
    pub next_hidden_state: &'a Buffer,
...
impl GatedMoE {
    ...
    fn validate_bucketed_input(&self, input: &GatedMoEBucketedReplayInput<'_>) {
        GatedMoEReplayShape {
    ...
    pub fn record_bucketed<'a, R>(&'a self, recorder: &mut R, input: GatedMoEBucketedReplayInput<'a>) -> &'a Buffer
    where
        R: Recorder<'a, Operator = ReplayOp<'a>>,
    {
    ...
    fn record_token_major_bucketed<'a>(
        ...
        input: GatedMoEBucketedReplayInput<'a>,
    ) {
        let num_total_tokens = input.num_total_tokens;
        let num_active_tokens_key = input.num_active_tokens_key;
        let shape = GatedMoEReplayShape {
        ...
        builder.record_with_barrier_before(ReplayOp::opaque(self.topk_experts.invoke_token_major_bucketed(
            num_total_tokens,
        ...
        match shared_experts {
            None => {
                builder.record_with_barrier_before(ReplayOp::opaque(
                    self.combine.invoke_without_shared_experts_bucketed(
            ...
            Some(shared_experts) => {
                ...
                builder.record_with_barrier_before(ReplayOp::opaque(self.combine.invoke_with_shared_experts_bucketed(
                    self.combine_shape(shape),
                    num_active_tokens_key,
    ...
    fn record_expert_major_bucketed<'a>(
        ...
        let layout = ReplayOp::opaque(self.expert_major.invoke_layout_bucketed(
            expert_major_shape,
            num_active_tokens_key,
        ...
        builder.record_with_barrier_before(ReplayOp::opaque(self.expert_major.invoke_pack_input_bucketed(
            expert_major_shape,
            num_active_tokens_key,
        ...
        builder.record_with_barrier_before(ReplayOp::opaque(self.topk_experts.invoke_expert_major_bucketed(
            num_total_tokens,
            self.num_experts_per_token(),
            num_active_tokens_key,
        ...
        match shared_experts {
            None => {
                builder.record_with_barrier_before(ReplayOp::opaque(
                    self.expert_major.invoke_scatter_without_shared_experts_bucketed(
                        expert_major_shape,
                        num_active_tokens_key,
            ...
            Some(shared_experts) => {
                builder.record_with_barrier_before(ReplayOp::opaque(
                    self.expert_major.invoke_scatter_with_shared_experts_bucketed(
                        expert_major_shape,
                        num_active_tokens_key,
        ...
        I: Recorder<'a, Operator = ReplayOp<'a>>,
    {
        let shared_experts = self
            ...
            .as_ref()
            .expect("gated MoE shared replay requires a configured shared expert");
        builder.record(ReplayOp::opaque(shared_experts.mlp.invoke_bucketed(
            num_total_tokens,
            num_active_tokens_key,
            QuantizedDenseMLPBuffers {
                hidden_state: input,
        ...
        builder.record(ReplayOp::opaque(shared_experts.shared_expert_gate.invoke_bucketed(
            num_total_tokens,
            num_active_tokens_key,
            scratch.gate_logits,
            0,
...
mod tests {
    ...
    fn test_routing_bucketed_chain_reuses_one_parameter_and_preserves_inactive_rows() {
        ...
        assert_invalid_active_token_arguments(&stream, &bucketed_replay, num_total_tokens);
        runtime
    ...
    #[test]
    fn test_full_bucket_policy_preserves_complete_topology() {
        let device = Device::system_default();

        for with_shared_experts in [false, true] {
            let (core, metal) = full_test_config(with_shared_experts);
            let moe = GatedMoE::new(&device, core, metal);
            let boundaries = moe.replay_topology_boundaries();
            let policy = ReplayBucketPolicy::with_topology_boundaries(64, &boundaries);

            assert!(boundaries.contains(&EXPERT_MAJOR_MIN_TOKENS));
            assert_eq!(
                moe.replay_topology(TOKEN_MAJOR_MAX_TOKENS).compute_path,
                GatedMoEComputePath::TokenMajor
            );
            assert_eq!(
                moe.replay_topology(EXPERT_MAJOR_MIN_TOKENS).compute_path,
                GatedMoEComputePath::ExpertMajor
            );
            assert_eq!(
                moe.replay_topology(1).shared_expert_gate_affine.is_some(),
                with_shared_experts
            );
            assert_eq!(
                moe.replay_topology(1).shared_experts_dense.is_some(),
                with_shared_experts
            );
            ...
            for num_active_tokens in 1..=64 {
                let num_total_tokens = policy.capacity(num_active_tokens);
                assert_eq!(
                    moe.replay_topology(num_active_tokens),
                    moe.replay_topology(num_total_tokens),
                    "with_shared_experts={with_shared_experts} num_active_tokens={num_active_tokens} \
    ...
    fn test_full_bucketed_replay_matches_exact_and_preserves_inactive_rows() {
        ...
        for with_shared_experts in [false, true] {
            ...
            for (num_active_tokens, num_total_tokens) in [(3_u32, 4_u32), (5_u32, 6_u32)] {
                ...
                assert_eq!(exact_active_replay.stats().parameter_count, 0);
                ...
                assert_eq!(exact_total_replay.stats().parameter_count, 0);
                ...
                if !with_shared_experts && num_total_tokens == 4 {
                    assert_invalid_active_token_arguments(&stream, &bucketed_replay, num_total_tokens);
                }
    ...
    #[test]
    fn test_full_bucketed_output_validation_uses_total_tokens() {
        let device = Device::system_default();
        let stream = inference_backend_metal::metal::Stream::new(&device);
        let runtime = MetalReplayRuntime::new(&stream);
        let (core, metal) = full_test_config(false);
        let moe = GatedMoE::new(&device, core.clone(), metal);
        let weights = FullMoETestWeights::new(&device, &core, metal);
        let max_tokens = 6_u32;
        let scratch = MoEScratch::new(&device, &core, metal, max_tokens as usize);
        let hidden_state = Buffer::new_zeroed_elements(&device, max_tokens as usize * core.hidden_dim, Dtype::Bfloat16);

        for (num_active_tokens, num_total_tokens) in [(3_u32, 4_u32), (5_u32, 6_u32)] {
            let short_output =
                Buffer::new_zeroed_elements(&device, num_active_tokens as usize * core.hidden_dim, Dtype::Bfloat16);
            assert_panics(|| {
                let mut recorder = runtime.create_recorder();
                let _ = moe.record_bucketed(
                    &mut recorder,
                    weights.bucketed_input(&scratch, &hidden_state, &short_output, num_total_tokens),
                );
            });
        }
    }
    ...
    impl FullMoETestWeights {
        ...
        fn shared_experts_input<'a>(&'a self, scratch: &'a MoEScratch) -> Option<GatedMoESharedExpertsReplayInput<'a>> {
    ...
    fn assert_invalid_active_token_arguments(
