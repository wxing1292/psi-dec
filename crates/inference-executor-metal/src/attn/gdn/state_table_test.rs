...
#[test]
fn test_replay_commit_and_publish_share_one_submission() {
    ...
    let boundary_pages = (0..num_pages).collect::<Vec<_>>();
    let rejected_pages = (num_pages..2 * num_pages).collect::<Vec<_>>();
    prepare_state(
        &state,
        &metadata,
        &[0, 1],
        ...
        &[0, 3],
        0,
        &[Vec::new(), vec![boundary_pages, rejected_pages]],
    );
    ...
    for layer in 0..2 {
        ...
        log.qkv
            .write_typed(start * 12, &[bf16::from_f32(layer as f32 + 1.0).to_bits(); 60]);
    }
    ...
    assert!(state.record_commit(&mut recorder, &jobs));
    assert!(state.record_publish(&mut recorder, &pages));
    runtime.submit_replay(&recorder.build()).wait();
    ...
    for layer in 0..2 {
        ...
        let page_start = layer * 5 * LIFECYCLE_PAGE_BYTES / 2;
        assert_eq!(
        ...
        assert_eq!(
            ...
            vec![bf16::from_f32(layer as f32 + 1.0).to_bits(); 24]
        );
    }
    assert_eq!(
