...
impl GQAMetalConfig {
    pub fn validate(self) {
        ...
        assert!(self.single_q_token_kv_token_tile_size > 0 && self.single_q_token_kv_token_tile_size <= 1024);
        assert!(
            self.single_q_token_num_threads_per_threadblock.is_power_of_two()
                && self.single_q_token_num_threads_per_threadblock <= 256
        );
        assert!(self.single_q_token_max_q_head_tile_size > 0 && self.single_q_token_max_q_head_tile_size <= 8);
        assert!(matches!(self.tiled_q_token_tile_size, 8 | 16));
    ...
    pub fn supports_tiled_q_tokens(self, core: &GQACore) -> bool {
    ...
    fn tiled_max_q_head_tile_size(self) -> usize {
        (256 / (self.tiled_q_token_tile_size / 8 * 32)) as usize
    }
}
...
impl GQA {
    ...
    fn sdpa_path(&self, num_tokens: u32, num_q_token_tiles: u32) -> GQASDPAPath {
        // Tiled SDPA uses Tq_tile=`tiled_q_token_tile_size`,
        // Tkv_tile=`tiled_kv_token_tile_size`, and Hq_tile=`q_head_tile_size`.
        // By average useful Q tokens per Tq tile, the selector uses the
        // single-Q-token path below 2, half-group Hq below 4, and full-group
        // Hq otherwise, subject to the 256-threadblock cap.
        assert!(num_tokens > 0);
        ...
        if !self.config.supports_tiled_q_tokens(&self.core) || (num_tokens as u64) < 2 * num_q_token_tiles as u64 {
            return GQASDPAPath::SingleQToken;
        }
...
impl ReplayLayer for GQA {
        ...
        R: Recorder<'a, Operator = ReplayOp<'a>>,
    {
        ...
        let attention_output = match self.sdpa_path(shape.num_tokens, shape.num_q_token_tiles) {
            ...
            GQASDPAPath::TiledQTokens { q_head_tile_size } => {
...
mod tests {
    ...
    fn test_sdpa_path() {
        ...
        let gqa_tiled_q_token_tile_8 = GQA::new(&Device::system_default(), core.clone(), config);

        assert_eq!(gqa_tiled_q_token_tile_8.sdpa_path(4, 4), GQASDPAPath::SingleQToken);
        assert_eq!(
            gqa_tiled_q_token_tile_8.sdpa_path(8, 4),
            GQASDPAPath::TiledQTokens { q_head_tile_size: 3 }
        );
        assert_eq!(
            gqa_tiled_q_token_tile_8.sdpa_path(16, 4),
            GQASDPAPath::TiledQTokens { q_head_tile_size: 6 }
        );

        let gqa_tiled_q_token_tile_16 = GQA::new(
            &Device::system_default(),
            core,
            GQAMetalConfig {
                tiled_q_token_tile_size: 16,
                ..config
            },
        );
        assert_eq!(
            gqa_tiled_q_token_tile_16.sdpa_path(32, 2),
            GQASDPAPath::TiledQTokens { q_head_tile_size: 4 }
        );
    }
}
