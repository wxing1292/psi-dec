...
const DEFAULT_MAX_QUEUED_REQUESTS: usize = 32;
const DEFAULT_MAX_RUNNING_REQUESTS: usize = 8;
...
fn run_qwen35(kind: Qwen35ModelKind, config: Qwen35ServerConfig) {
    ...
    let max_queued_requests = DEFAULT_MAX_QUEUED_REQUESTS;
    let max_running_requests = DEFAULT_MAX_RUNNING_REQUESTS;
    assert!(
        scheduler_config.max_requests <= max_running_requests,
        "scheduler max_requests={} exceeds runtime max_running_requests={max_running_requests}",
    ...
    let runtime_config = build_metal_runtime(
        &startup,
        &model_config,
        max_queued_requests,
        max_running_requests,
...
fn run_qwen35_metal_runtime_n<const N: usize>(
    ...
    match num_mtp_modules + 1 {
        1 => run_replay_model_runtime::<N, 1, _>(listen_addr, runtime_config, scheduler_config, model, debug_logging),
        2 => run_replay_model_runtime::<N, 2, _>(listen_addr, runtime_config, scheduler_config, model, debug_logging),
