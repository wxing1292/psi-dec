    ...
    fn parse(args: impl IntoIterator<Item = std::ffi::OsString>) -> Result<Self> {
        ...
        while let Some(argument) = args.next() {
            ...
            match name {
                ...
                "--high-bits" => high_bits = parse_usize(name, &value)?,
...
fn usage() -> &'static str {
    "usage: qwen3_dflash2_quantize --input-dir DIR --output-dir DIR [--group-size 64] [--bits 4] [--high-bits 6]"
}
...
fn quantize_checkpoint(input_dir: &Path, output_dir: &Path, options: QuantizeOptions) -> Result<()> {
    ...
    let input_config_bytes = std::fs::read(&input_config_path)
        .map_err(|err| error(format!("unable to read DFlash2 config {input_config_path:?}: {err}")))?;
    let config = init_qwen3x_dflash2_config(input_dir)
        .map_err(|err| error(format!("invalid Qwen3x DFlash2 config {input_config_path:?}: {err}")))?;
    let mut config_value = serde_json::from_slice::<serde_json::Value>(&input_config_bytes).map_err(|err| {
        error(format!(
            "unable to preserve DFlash2 config {input_config_path:?}: {err}"
        ))
    })?;
    ...
    if !input_checkpoint.is_file() {
        return Err(error(format!(
            "DFlash2 converter currently requires the linked single-file checkpoint at {input_checkpoint:?}"
        )));
    }
    ...
    write_output_config(&mut config_value, &temp_dir.join("config.json"), &config, options)?;
    std::fs::rename(&temp_dir, output_dir).map_err(|err| {
        error(format!(
            "unable to publish quantized DFlash2 checkpoint {temp_dir:?} as {output_dir:?}: {err}"
        ))
    })?;
...
fn validate_options(options: QuantizeOptions) -> Result<()> {
    ...
    for (name, bits) in [("bits", options.bits), ("high_bits", options.high_bits)] {
...
fn quantize_safetensors(
    ...
    let mut input = File::open(input_path)
        .map_err(|err| error(format!("unable to open BF16 DFlash2 checkpoint {input_path:?}: {err}")))?;
    ...
    let metadata = HashMap::from([
        ("format".to_string(), "psi-dec-dflash2-affine".to_string()),
        ("group_size".to_string(), options.group_size.to_string()),
        ("bits".to_string(), options.bits.to_string()),
        ("high_bits".to_string(), options.high_bits.to_string()),
    ]);
    ...
    for (name, info) in &header.tensors {
        ...
        let output_name = canonical_output_name(name);
        if info.shape.len() == 2 {
...
fn validate_source_tensors(
    ...
    config: &Qwen3xDFlash2Config,
    path: &Path,
) -> Result<Qwen3xDFlash2WeightBindings> {
    let actual = header
        .tensors
        .keys()
        .cloned()
        .collect::<std::collections::BTreeSet<_>>();
    let bindings = resolve_qwen3x_dflash2_source_weight_bindings(config, actual.iter().map(String::as_str))
        .map_err(|err| error(format!("Qwen3x DFlash2 checkpoint {path:?} tensor set mismatch: {err}")))?;
    for (name, info) in &header.tensors {
        if info.dtype != Dtype::BF16 {
            return Err(error(format!(
                "DFlash2 source tensor {name:?} must be BF16, found {:?}",
                info.dtype
            )));
        }
        if !matches!(info.shape.len(), 1 | 2 | 3) {
            return Err(error(format!(
                "DFlash2 source tensor {name:?} must be rank 1, 2, or 3, shape={:?}",
                info.shape
            )));
        }
    }
...
fn validate_output_tensor_names(output_tensors: &[OutputTensor], bindings: &Qwen3xDFlash2WeightBindings) -> Result<()> {
    ...
    if actual != expected {
        ...
        return Err(error(format!(
            "quantized DFlash2 tensor set mismatch: missing={missing:?} unexpected={unexpected:?}"
        )));
    }
    Ok(())
}
...
fn build_output_tensors(header: &SafetensorsHeader, options: QuantizeOptions) -> Result<Vec<OutputTensor>> {
    ...
    for (name, info) in &header.tensors {
        ...
        if info.shape.len() == 2 {
            ...
            tensors.push(output_tensor(output_name.clone(), Dtype::U32, packed_shape)?);
...
fn quantize_bf16_matrix(
    ...
    let mut scales = vec![0u8; rows * groups_per_row * 4];
    let mut biases = vec![0u8; rows * groups_per_row * 4];
    ...
    for row in 0..rows {
        for group in 0..groups_per_row {
            ...
            write_f32(&mut scales, affine_index, scale);
            write_f32(&mut biases, affine_index, bias);
...
fn write_f32(data: &mut [u8], index: usize, value: f32) {
    data[index * 4..index * 4 + 4].copy_from_slice(&value.to_le_bytes());
}
...
fn read_f32(data: &[u8], index: usize) -> f32 {
    f32::from_le_bytes(
        data[index * 4..index * 4 + 4]
            .try_into()
            .expect("F32 slice length is fixed"),
    )
}
...
fn write_output_config(
    ...
    if options.high_bits != options.bits {
        for name in HIGH_BIT_TENSORS
            .into_iter()
            .filter(|name| high_bit_layer_index(name) < config.num_hidden_layers)
        {
            let name = name.strip_suffix(".weight").expect("high-bit tensor must be a weight");
            quantization.insert(name.to_string(), serde_json::json!({ "bits": options.high_bits }));
        }
    }
    object.insert("quantization".to_string(), serde_json::Value::Object(quantization));
    let bytes = serde_json::to_vec_pretty(config_value)
        .map_err(|err| error(format!("unable to encode quantized DFlash2 config: {err}")))?;
    std::fs::write(path, bytes)
        .map_err(|err| error(format!("unable to write quantized DFlash2 config {path:?}: {err}")))?;
    Ok(())
}
...
fn write_safetensors_index(checkpoint_path: &Path, index_path: &Path) -> Result<()> {
    let mut checkpoint = File::open(checkpoint_path).map_err(|err| {
        error(format!(
            "unable to open affine DFlash2 checkpoint {checkpoint_path:?}: {err}"
        ))
    })?;
    ...
    let bytes = serde_json::to_vec_pretty(&index)
        .map_err(|err| error(format!("unable to encode affine DFlash2 safetensors index: {err}")))?;
    std::fs::write(index_path, bytes).map_err(|err| {
        error(format!(
            "unable to write affine DFlash2 safetensors index {index_path:?}: {err}"
        ))
    })?;
    Ok(())
}
...
mod tests {
    ...
    fn test_quantize_matrix_matches_affine_contract() {
        ...
        for row in 0..2 {
            let scale = read_f32(&scales, row);
            let bias = read_f32(&biases, row);
            for column in 0..64 {
    ...
    fn test_tiny_checkpoint_round_trip() {
        ...
        .unwrap();
        let parsed = init_qwen3x_dflash2_config(&input_dir).unwrap();
        ...
        assert_eq!(checkpoint.tensor("fc.scales").unwrap().dtype(), Dtype::F32);
        assert_eq!(
    ...
    fn tiny_tensors(config: &Qwen3xDFlash2Config) -> BTreeMap<String, OwnedTensor> {
        let mut tensors = BTreeMap::new();
        insert_matrix(
            &mut tensors,
            "fc.weight",
    ...
    fn insert_base_kernel(tensors: &mut BTreeMap<String, OwnedTensor>, name: &str, taps: usize, hidden_size: usize) {
        let values = vec![0.5; 2 * taps * hidden_size];
        tensors.insert(
            name.to_string(),
            OwnedTensor {
                dtype: Dtype::BF16,
